...
//import vaulsys.scheduler.job.CyclePerTransactionCycleSettlementJob;
...
public class AddClearingProfile {
    ...
    private static final String DBINITIALIZER_USER = "FNS_DBInitializer";
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            clearingProfile.setAccountingGuaranteeMinute(-300);

            SettlementDataCriteria stlCriteria = new SettlementDataCriteria(SettlementDataType.MAIN);
            clearingProfile.setName("یک بار در روز پرداخت قبض در هر شعبه");
            clearingProfile.setId(170617L);
            stlCriteria.addCriteriaDatas(TrnType.class, TrnType.BILLPAYMENT.getType());
            stlCriteria.addCriteriaDatas(IfxType.class, IfxType.BILL_PMT_RS.getType());
            ...
            stlCriteria.setDocDesc("تسويه پرداخت قبض در هر شعبه");
            GeneralDao.Instance.saveOrUpdate(stlCriteria);
            clearingProfile.addCriterias(stlCriteria);
            clearingProfile.setHasFee(false);

            clearingProfile.setSettlementClass(PerTransactionKioskBillPaymentSettlementServiceImpl.class);
            scheduleClearingProfile(clearingProfile, 20, -1, 23, 0);
            DateTime fireTime = new DateTime(new DayDate(0, 0, 0), new DayTime(3, 30, 0));
            ...
            clearingProfile.setCreatorUser(getUser());
            clearingProfile.setCreatedDateTime(DateTime.now());

            clearingProfile.setFireTime(fireTime);
            clearingProfile.setCycleCriteria(criteria, new PerTransactionKioskBillPaymentSettlementJob());
...
//		createPerDayClearingProfile(new ATMDailyCycleSettlementJob());
...
//		
//		clrProf = GeneralDao.Instance.load(ClearingProfile.class, 170602L);
//		clrProf.setCycleCriteria(clrProf.getCycleCriteria(), new BillPaymentCycleSettlementJob());
//
//		ClearingProfile clrProf = GeneralDao.Instance.load(ClearingProfile.class, 170603L);
//		clrProf.setCycleCriteria(clrProf.getCycleCriteria(), new CellChargeCycleSettlementJob());
//
        ClearingProfile clrProf = GeneralDao.Instance.load(ClearingProfile.class, 2303701L);
...
//			stlCriteria.addCriteriaDatas(ClearingState.class, ClearingState.SUSPECTED_DISAGREEMENT.getState());
		...
		}*/ else if (job instanceof RequestBasedCycleSettlementJob) {
...
//			stlCriteria = new SettlementDataCriteria(SettlementDataType.SETTLE_TIME);
            ...
            fireTime = new DateTime(new DayDate(0, 0, 0), new DayTime(2, 0, 0));
        } else if (job instanceof OnlinePerTransactionCycleSettlementJob) {
...
//				stlCriteria = new SettlementDataCriteria(SettlementDataType.SETTLE_TIME);
    ...
    private static void createClearingProfileSyncObj(ClearingProfile clearingProfile) {
    ...
    private static void scheduleClearingProfile(ClearingProfile clearingProfile, Integer accountTimeOffsetMinute, Integer settleTimeOffsetDay,
