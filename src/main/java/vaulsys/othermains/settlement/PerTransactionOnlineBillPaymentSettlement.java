...
public class PerTransactionOnlineBillPaymentSettlement {
	...
	public static void main(String[] args) {
		...
		try {
			...
			clearingProfile = ClearingService.findClearingProfile(170609L);
...
//				PerTransactionSettlementServiceImpl.Instance.account(clearingProfile, clearingProfile.getAccountUntilTime(now), clearingProfile.getSettleUntilTime(now), false, false, false, false);
				PerTransactionOnlineBillSettlementServiceImpl.Instance.settle(clearingProfile, settleUntilTime, false, true, true);
			} catch (Exception e) {
