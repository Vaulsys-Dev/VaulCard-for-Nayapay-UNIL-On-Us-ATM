...
public class ATMSettlement {
	...
	private static void atmSettlement(String[] args) {
		...
		ClearingProfile clearingProfile = ClearingService.findClearingProfile(19137601L);
		if(args.length < 2){
...
//			 ATMSettlementServiceImpl.Instance.settle(clearingProfile, settleUntilTime, true);
			 try {
				ATMSettlementServiceImpl.Instance.account(terminals, clearingProfile, settleUntilTime, settleUntilTime, false, false, false, false, false);
			} catch (Exception e) {
				e.printStackTrace();
			} 
		 }
