...
public class OnlineSettlement {
	public static final Logger logger = Logger.getLogger(OnlineSettlement.class);
...
//			terminals = GeneralDao.Instance.find("from POSTerminal where code in (207796)");
		...
		while(numTries < maxTries && !isFinishedAccounting){
			try{
				OnlineSettlementService.Instance.account(terminals, clearingProfile, clearingProfile.getAccountUntilTime(now), clearingProfile.getSettleUntilTime(afterNow), false, false, false, true, false);
