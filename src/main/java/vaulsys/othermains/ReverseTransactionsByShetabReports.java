...
public class ReverseTransactionsByShetabReports {
...
//			fileName = path + "/" +ShetabReconciliationService.REVERSAL_FILES[i];
			...
			try {
				...
				ShetabReconciliationService.getListOfTransactionInShetabReversalReport(new BufferedReader(new FileReader(shetabReport)), new BufferedWriter(new FileWriter(shetabReportRes)), null, GlobalContext.getInstance().getSwitchUser());
			} catch (Exception e) {
				e.printStackTrace();
			}
