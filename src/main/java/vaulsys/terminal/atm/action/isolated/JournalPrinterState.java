...
public class JournalPrinterState extends IsolatedState {
	public static final JournalPrinterState Instance = new JournalPrinterState();

	private JournalPrinterState(){}
	...
	protected Message process(Message inputMessage, ATMTerminal atm) {
		inputMessage.getIfx().setIfxType(IfxType.JOURNAL_PRINTER_STATE);
		setDebugTag(inputMessage.getTransaction());
		ATMTerminalService.updateJournalPrinter(atm, inputMessage);
...
//			return null;
		...
		outMsg.setProtocolMessage(ATMTerminalService.generateSendConfigInfoMessage(ndcMsg.logicalUnitNumber,
				NDCTerminalCommandModifierConfigurationInfo.SEND_FITNESS_DATA_ONLY));
