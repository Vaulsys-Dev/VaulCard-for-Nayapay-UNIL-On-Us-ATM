...
public class FSM {
    ...
    private Logger logger = Logger.getLogger(FSM.class);
    ...
    public byte[] generateMAC(int index, byte[] keyData, byte[] data, int algorithm, byte[] IV, int macLength, int padding, byte[] masterKey) {
        ...
        //logger.info("using key:" + new String(Hex.encode(keyData)).toUpperCase());
...
//		logger.debug("Received PIN:" + HSMUtil.hexToString(pinBlock));
