...
public class ATMSettlementServiceImpl extends SettlementService {
	private static final Logger logger = Logger.getLogger(ATMSettlementServiceImpl.class);
	private static final int MAX_COUNTER = 100;

	private ATMSettlementServiceImpl() {
	}

	public static final ATMSettlementServiceImpl Instance = new ATMSettlementServiceImpl();

	@Override
	public void account(List<Terminal> terminals, ClearingProfile clearingProfile, DateTime accountUntilTime,
			DateTime settleUntilTime, Boolean update, Boolean waitForSyncObject, Boolean onlyFanapAccount, Boolean settleTime, Boolean considerClearingProcessType) throws Exception {
...
//									List<Long> trxSettlemRecordList = new ArrayList<Long>();
									atmList.add(atm);
									prepareForSettlement(clearingProfile, atmList, accountUntilTime, settleUntilTime, settleTime, true/*considerClearingProcessType*//*, trxSettlemRecordList*/, waitForSyncObject);
...
//									SynchronizationService.release(atm, ATMTerminal.class);
						...
						if (freeTerminals != null && freeTerminals.size() > 0) {
							try {
								...
								postPrepareForSettlement(clearingProfile, settleUntilTime, freeTerminals, update, accountUntilTime);
...
//        deleteFromSettlementRecord(clearingProfile);
	}
	...
	List<String> findDesiredTerminalCodes(DateTime accountUntilTime, Boolean justToday, ClearingProfile clearingProfile) {
		...
		List<String> atmTerminals = TerminalService.findAllTerminalsWithTrxUntilTime(ATMTerminal.class,	accountUntilTime, justToday, guaranteePeriod);
		if (atmTerminals != null && atmTerminals.size() > 0)
			terminals.addAll(atmTerminals);
		return terminals;
	}
...
//		if (!OnlineSettlementService.class.equals(clearingProfile.getSettlementClass())) {
		while (numTries < maxTries && !isFinishedAccounting) {
			try {
//				account(terminals, clearingProfile, settleUntilTime, settleUntilTime, update, true, false, settleTime, false);
                account(terminals, clearingProfile, settleUntilTime, settleUntilTime, update, true, false, settleTime, true);
...
//	@NotUsed
	...
	protected void generateDocumentSettlementState(ClearingProfile clearingProfile, String docDesc, DateTime settleDate)
		...
		try {
			generateATMDesiredSettlementReports(settlementDatas, false, null, settleDate, null, clearingProfile);
		} catch (Exception e) {
...
//			} catch (Exception e) {
//				logger.error("Error was occured in Shetab Settlement. (" + e.getClass().getSimpleName() + ": "
//						+ e.getMessage() + ")");
//			}
//
//		}
...
//	}
	...
	private Object postPrepareForSettlement(ClearingProfile clearingProfile, DateTime settleDate, List<Terminal> terminals, Boolean update, DateTime accountUntilTime) {
		List<SettlementData> settlementDatas = AccountingService.findAllNotSettledATMSettlementDataUntilTime(
				clearingProfile, settleDate, terminals);
		if (update) {
			try {
				generateATMDesiredSettlementReports(settlementDatas, update, terminals, settleDate, accountUntilTime, clearingProfile);
//				generateReportDocument(settlementDatas.get(0), update, terminals, settleDate, accountUntilTime, clearingProfile);
			} catch (Exception e) {
...
//				generateATMDesiredSettlementReports(settlementData);
	...
	private void generateReportDocument(SettlementData settlementData, Boolean update, List<Terminal> terminals, DateTime settleDate, DateTime accountUntilTime, ClearingProfile clearingProfile) throws BusinessException {
		if (settlementData.getSettlementReport() != null) {
			logger.warn("settlementData: " + settlementData.getId() + " has been report: "+ settlementData.getSettlementReport().getId() + " !!!");
			return;
		}
		...
		ATMTerminal atm = GeneralDao.Instance.load(ATMTerminal.class, terminal.getId());
...
//		Account fanapAccount = GlobalContext.getInstance().getMyInstitution().getAccount();
		...
		try {
			...
			ATMTerminal atm = GeneralDao.Instance.load(ATMTerminal.class, returnedSettlementData.getTerminal().getId());
	...
	public void generateATMReturnedReport(List<SettlementInfo> settlementInfoList, String accountNumber, String commentOfDoc) throws Exception {
		SettlementInfo settlementInfo = settlementInfoList.get(0);
		...
		if (Util.hasText(commentOfDoc)) {
			clrProfDesc = commentOfDoc;
		}
		...
		if (AccountingService.isTopic(accountNumber))
			topic = DocumentItemEntityType.Topic;
		else
			topic = DocumentItemEntityType.Account;
		...
		DocumentItemEntity documentItemEntity = new DocumentItemEntity(new Double(returnedSettlementData
				.getTotalSettlementAmount()), debtor, switchBranchId, commentOfDocumentItem, accountNumber, topic);
		// -------------------
//		IssueGeneralDocVO issueGeneralDocVO = new IssueGeneralDocVO(commentOfDocumentItem);
//		issueGeneralDocVO.getDeposits().add(deposit);
//		issueGeneralDocVO.getEntities().add(documentItemEntity);
