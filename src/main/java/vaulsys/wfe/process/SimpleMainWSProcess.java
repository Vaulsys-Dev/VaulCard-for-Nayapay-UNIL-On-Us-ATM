...
public class SimpleMainWSProcess {
    ...
    public WalletCMSWsEntity processTransaction() {
        try {
            ...
            //m.rehman: 27-06-2022, VP-NAP-202206221 - Cash WithDrawal with repeated reference numbers - Production
            String query = "from " + WalletCMSWsEntity.class.getName() + " a where " +
                    "a.tranrefnumber = :TXN_REF_NO " +
                    "and a.channelid = :CHANNEL_ID ";
            Map<String, Object> params = new HashMap<>();
            params.put("TXN_REF_NO", wsobj.getTranrefnumber());
            params.put("CHANNEL_ID", wsobj.getChannelid());
            WalletCMSWsEntity entity = (WalletCMSWsEntity) GeneralDao.Instance.findObject(query, params);
            if (entity != null) {
                logger.error("Transaction with reference number [" + wsobj.getTranrefnumber() + "] and Channel ID [" + wsobj.getChannelid() + "] already exist, rejecting ...");
                wsobj.setRespcode(ISOResponseCodes.DUPLICATE_TRANSACTION);
                return wsobj;
                //throw new AuthorizationException("Unable to Authorize Transaction");
            }
            ///////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            switch(servicename)
            {
                ...
                case "OnUsCashWithDrawalInquiry":
                {
                    WebServiceHandler.processOnUsCashWithDrawalInquiry(wsobj);
                    break;
                }
                // ==================================================================
