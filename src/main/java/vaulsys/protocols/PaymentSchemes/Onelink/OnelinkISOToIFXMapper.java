 ...
 */
public class OnelinkISOToIFXMapper extends ISOtoIfxMapper {

    public static final OnelinkISOToIFXMapper Instance = new OnelinkISOToIFXMapper();

    private OnelinkISOToIFXMapper() {}
    ...
    public Ifx map(ProtocolMessage message, EncodingConvertor convertor) throws Exception {
        ...
        //m.rehman: unable to set terminal type through given function
        //mapTerminalType(ifxObj, f_25);
        ifxObj.setTerminalType(TerminalType.SWITCH);
        ...
        if(Util.hasText(isoMsg.getString(32))) {
            //Raza TEMPORARY HARDCODING LENGTH AS 6, will update it later
            if(isoMsg.getString(32).contains(ProcessContext.get().getInputMessage().getChannel().getInstitutionId())) { //Raza TODO improve it
                ifxObj.setBankId(ProcessContext.get().getInputMessage().getChannel().getInstitutionId());
            }
            else
            {
                ifxObj.setBankId(isoMsg.getString(32));
            }
//            if(isoMsg.getString(32).length() > 6) {
//                ifxObj.setBankId(isoMsg.getString(32).substring(0,7)); //Raza Doing it as per Onelink Instructions they asked it is always of length 6
//            }
//            else {
//                ifxObj.setBankId(isoMsg.getString(32));
//            }
        ...
        //ifxObj.setMyNetworkRefId(isoMsg.getString(37)); //commenitng only using NetworkRefId
        ...
        ifxObj.setTerminalId(isoMsg.getString(41).trim());

        ifxObj.setOrgIdNum(isoMsg.getString(42).trim());
        ...
        mapOnelinkIfxType(ifxObj, mti, emvTrnType);
    ...
    private void mapOnelinkIfxType(Ifx ifx, String mti, String emvTrnType) {
        if (mti.equals(ISOMessageTypes.FINANCIAL_ADVICE_RESPONSE_87)
            ...
            if (emvTrnType.equals(ISOTransactionCodes.PURCHASE))
                ifx.setIfxType(IfxType.PREAUTH_COMPLET_ADVICE_RS);
        } else {
