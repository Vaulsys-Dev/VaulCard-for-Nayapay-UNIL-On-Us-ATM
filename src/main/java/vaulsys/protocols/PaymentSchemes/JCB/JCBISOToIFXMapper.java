 ...
 */
public class JCBISOToIFXMapper extends ISOtoIfxMapper {

    public static final JCBISOToIFXMapper Instance = new JCBISOToIFXMapper();

    private JCBISOToIFXMapper() {}
    ...
    public Ifx map(ProtocolMessage message, EncodingConvertor convertor) throws Exception {
                ...
                if (ISOMessageTypes.isRequestMessage(mti)) {
                    if (ISOTransactionCodes.isFinancialTransaction(emvTrnType) && !(emvTrnType.equals(ISOTransactionCodes.REFUND))) {
                        ...
                        if (ifxObj.getCurrCodeSettlement() != null) {
                            if (ifxObj.getCurrCodeSettlement().equals(GlobalContext.getInstance().getBaseCurrency().getCode())) {
                                ifxObj.setSec_Amt(ifxObj.getAmountSettlement());
                                ifxObj.setSec_Currency(Integer.parseInt(ifxObj.getCurrCodeSettlement()));
                                ifxObj.setSec_CurRate(ifxObj.getConvRateSettlement());

                            } else if (!ifxObj.getCurrCodeSettlement().equals(GlobalContext.getInstance().getBaseCurrency().getCode())) {
                                currency = ProcessContext.get().getCurrency(Integer.parseInt(ifxObj.getCurrCodeSettlement()));
                                convertedAmount = ifxObj.getAmountSettlement() * currency.getExchangeRate();
        ...
        */
        ...
        if(Util.hasText(isoMsg.getString(33))) {
            ifxObj.setFwdBankId(isoMsg.getString(33));
        }

        if (Util.hasText(isoMsg.getString(35))) {
       ...
       //ifxObj.setVipPrivateUse(isoMsg.getString(63)); //Raza present in TPSP
            ifxObj.setNetworkData(isoMsg.getString(63));
        }
        ...
        if (Util.hasText(isoMsg.getString(100))) {
            ifxObj.setRecvBankId(isoMsg.getString(100));
        }

        if (Util.hasText(isoMsg.getString(101))) {
            ifxObj.setAccountId1(isoMsg.getString(101));
        }
        ...
        */
        ...
        if (Util.hasText(isoMsg.getString(105))) {
            ifxObj.setMesgSecurityCode(isoMsg.getString(105));
        }
